...
import { FimAccountJson, FimExpenseJson, FimGoalJson, FimIncomeJson, FimModelJson, FimReccurenceJson } from "./model-json";
import { createMontlyReccurance } from "../montly-recurrence";
import { createFimModel, FimAccount, FimExpense, FimGoal, FimIncome, FimModel } from "../../model";

export const createAccountFromJson = (json: FimAccountJson): FimAccount => ({
        deposit: (_name,amount) => {json.balance += amount;},
        withdraw: (_name, amount) => {json.balance -= amount;},
...
export const createRecurrenceFromJson = (json: FimReccurenceJson) => {
    switch(json.type) {
        case "monthly": return createMontlyReccurance(json.dayOfMonth);
        default: throw new Error(`Unknown recurrence type [${json.type}].`);
    };
};

const verifyGoalFromJson = (json: FimGoalJson, accounts: FimAccount[]) => {
    switch(json.type) {
        case "all-debit-accounts-have-positive-balance": return verifyNoOverDraftGoal(accounts);
        default: throw new Error(`Unknown goal type: [${json.type}]`);
    }
}

const createGoalFromJson = (json: FimGoalJson, accounts: FimAccount[]): FimGoal=> {
...
export const createFimModelFromJson = (json: FimModelJson): FimModel => {
    ...
    const getAccountByName = (() => {
        const accountsMap = new Map(accounts.map(account => ([account.getName(), account])));
        return (name: string) => {
    ...
    const expenses = json.expenses.map(expenseJson => createExpenseFromJson(expenseJson, getAccountByName));
    const incomes = json.incomes.map(incomeJson => createIncomeFromJson(incomeJson, getAccountByName));
    const goals = json.goals.map(goalJson => createGoalFromJson(goalJson, accounts));
