import { createFimModel, FimAccount, FimExpense, FimGoal, FimGoalResult, FimIncome, FimReccurance, FimResult } from "./model";
...
describe("model.verify", () => {
    ...
    describe("processes goals", () => {
        ...
        it("failing", () => {
            const failingVerifyMock = 
                jest.fn<FimGoalResult, []>()
                .mockImplementationOnce(() => ({type: "passed"}))
                .mockImplementationOnce(() => ({type: "failed", message: "failed-message"}));
            ...
            const model = createFimModel([], [], [], [testFailingGoal, testPassingGoal]);
            const result = model.verify(testStart, testEnd);
