import { FimAccountJson, FimAccountType, FimExpenseJson, FimGoalJson, FimIncomeJson, FimModelJson } from "./recurrences/formats/model-json";
            ...
            while (current <= end) {
                for (const income of incomes) {
                    if (income.getRecurrence().occursOn(current)) {
                        income.getInto().deposit(income.getName(), income.getAmount(), current);
                    }
                }
                for (const expense of expenses) {
                    if (expense.getRecurrence().occursOn(current)) {
                        expense.getFrom().withdraw(expense.getName(), expense.getAmount(), current);
                    }
                }
